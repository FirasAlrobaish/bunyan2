...
import { ArrowRight, Plus, Share2, TrendingUp, TrendingDown, Wallet, Search, Trash2, Image, X, Check, Upload, Eye, Tag, BarChart2, FileText, PieChart } from 'lucide-react'
...
export default function ProjectPage() {
  ...
  const [activeView, setActiveView] = useState<'list'|'charts'|'categories'>('list')
  ...
  const byCat = (type: 'INCOME'|'EXPENSE') => Object.entries(
    transactions.filter(t => t.type===type).reduce((acc,t) => { const k=t.category||'أخرى'; acc[k]=(acc[k]||0)+t.amount; return acc }, {} as Record<string,number>)
    ...
    <div className="min-h-screen" style={{background:'#0f1117'}}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl w-fit" style={{background:'rgba(255,255,255,0.04)'}}>
          {([{key:'list',label:'السجل',Icon:Wallet},{key:'charts',label:'التقارير',Icon:BarChart2},{key:'categories',label:'الفئات',Icon:Tag}] as const).map(({key,label,Icon}) => (
        ...
        {activeView==='list' && <>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex gap-1 p-1 rounded-xl" style={{background:'rgba(255,255,255,0.04)'}}>
              {(['all','INCOME','EXPENSE'] as const).map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all" style={activeTab===tab?{background:'linear-gradient(135deg,#c9a96e,#a07d54)',color:'#0f1117'}:{color:'rgba(245,240,232,0.4)'}}>{tab==='all'?'الكل':tab==='INCOME'?'إيرادات':'مصاريف'}</button>)}
            </div>
            {isOwner && <button onClick={()=>setShowForm(true)} className="btn-primary flex items-center gap-2 !py-2.5"><Plus size={16}/>إضافة</button>}
          </div>
            ...
            : filtered.map((t,i) => (
              <div key={t.id} className="card !p-4 flex items-center gap-4 group fade-in hover:border-white/10 transition-all" style={{animationDelay:`${i*0.03}s`}}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isOwner&&<button onClick={()=>deleteTransaction(t.id)} className="opacity-0 group-hover:opacity-30 hover:!opacity-70 transition-opacity text-red-400"><Trash2 size={14}/></button>}
                </div>
              </div>
            ))}
          </div>
        ...
        {activeView==='categories' && <div className="fade-in space-y-4">
          {isOwner && <div className="card"><h3 className="font-bold mb-4 flex items-center gap-2"><Plus size={16} style={{color:'#c9a96e'}}/>إضافة فئة جديدة</h3>
            <div className="flex gap-3 flex-wrap items-center">
              ...
              <select value={catForm.type} onChange={e=>setCatForm(f=>({...f,type:e.target.value as any}))} className="input-field w-auto"><option value="EXPENSE">مصروف</option><option value="INCOME">إيراد</option></select>
          ...
          <div className="card"><h3 className="font-bold mb-4 text-green-400">فئات الإيرادات</h3><div className="flex flex-wrap gap-2">
        ...
        <div className="card w-full max-w-lg max-h-[90vh] overflow-y-auto fade-in">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs opacity-50 mb-2 block">الكمية</label><input type="number" value={form.quantity} onChange={e=>{const q=e.target.value;const amt=q&&form.unit_price?String(parseFloat(q)*parseFloat(form.unit_price)):form.amount;setForm(f=>({...f,quantity:q,amount:amt}))}} className="input-field" placeholder="0"/></div>
              <div><label className="text-xs opacity-50 mb-2 block">سعر الوحدة</label><input type="number" value={form.unit_price} onChange={e=>{const p=e.target.value;const amt=form.quantity&&p?String(parseFloat(form.quantity)*parseFloat(p)):form.amount;setForm(f=>({...f,unit_price:p,amount:amt}))}} className="input-field" placeholder="0"/></div>
            </div>
            ...
            <div><label className="text-xs opacity-50 mb-2 block">إيصال / صورة</label><input ref={fileRef} type="file" accept="image/*,application/pdf" className="hidden" onChange={e=>setReceipt(e.target.files?.[0]||null)}/><button onClick={()=>fileRef.current?.click()} className="btn-ghost w-full flex items-center justify-center gap-2 text-sm"><Upload size={16}/>{receipt?receipt.name:'رفع ملف'}</button></div>
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={saveTransaction} disabled={!form.title||!form.amount||!form.date||saving} className="btn-primary flex-1 disabled:opacity-40 flex items-center justify-center gap-2">
                {saving||uploading?<><div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"/>{uploading?'رفع...':'حفظ...'}</>:<><Check size={16}/>حفظ</>}
              </button>
            </div>
          </div>
        </div>
